    ...
    pub async fn check_and_record(&self, from: &str, to: &str, message: &str) -> ErgataiResult<()> {
        ...
        match &conv.token_owner {
            ...
            TokenOwner::Held(ref previous_holder) => {
                ...
                // But first check if the sender has exceeded consecutive sends limit.
                let sender_consecutive = conv.consecutive_sends.get(from).copied().unwrap_or(0);
                if sender_consecutive >= self.config.max_consecutive_sends {
                    // Sender has used up their consecutive send allowance.
                    // They must wait for the other party to respond.
                    warn!(
                        conv_id = %conv.id,
                        from = from,
                        token_holder = %previous_holder,
                        consecutive_sends = sender_consecutive,
                        max_consecutive = self.config.max_consecutive_sends,
                        "Token held by other party — sender must wait for reply"
                    );
                    return Err(ErgataiError::internal(format!(
                        "Cannot send: token is held by {} (you've sent {} times consecutively, max {})",
                        previous_holder, sender_consecutive, self.config.max_consecutive_sends
                    )));
                }
        ...
        if has_terminate {
            ...
            // Transfer token to the receiver (the other party).
            conv.token_owner = TokenOwner::Held(to.to_string());
            info!(
                conv_id = %conv.id,
                from = from,
                to = to,
                "Token transferred to receiver (expecting reply)"
            );
