...
pub struct ErgataiMcpServer {
    ...
    /// Used to bind MCP connections to specific ACP agents
...
impl ErgataiMcpServer {
    #[tool(
        description = "List all available agents — both running and configured.\n\nRESPONSE: {agents: [{name, status, health, availability, can_receive_messages, ...}], total, running_count, configured_count}\n\nSTATUS VALUES:\n- `status: \"running\"` — Agent is running and can receive messages immediately\n- `status: \"configured\"` — Agent template from profile registry; will be auto-spawned when you send it a message\n\nUSAGE:\n- To message a running agent: use `ergatai_agent_id` as `target_agent_id` in `send_message`\n- To message a configured agent: use `name` (profile name) as `target_agent_id` — system auto-spawns it\n- Check `can_receive_messages: true` to confirm the agent can accept messages NOW\n\nFILTER: pass `filter` as JSON object, e.g. {\"filter\": {\"status\": \"running\"}} or {\"filter\": {\"in_dag\": \"dag-1\"}}.",
