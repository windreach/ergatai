...
impl ErgataiMcpServer {
    #[tool(
        description = "List available agents in Ergatai. Use BEFORE `send_message` to discover valid target_agent_id values and check if agents can receive messages. Excludes the caller automatically.\n\nSEMANTICS (IMPORTANT):\n- `health`: Process status — \"healthy\" means the agent process is alive AND ACP connection is established. \"unhealthy\" means process crashed or connection lost. Note: Health is verified when messages are actually sent; an agent marked healthy may still fail if it cannot process messages (e.g., API errors).\n- `availability`: Whether the agent can receive messages NOW — \"available\" means idle and ready, \"busy\" means currently processing a task, \"idle\" means waiting for work, \"unavailable\" means process is not alive.\n- `can_receive_messages`: Direct boolean — true means the agent is alive and not currently processing, so you can send a message.\n- DO NOT interpret \"state: running\" as \"busy\" — it only means the process is running. Check `availability` or `can_receive_messages` to know if the agent is free.\n\nRESPONSE: JSON with {agents: [{agent_id, health, availability, can_receive_messages, state, workspace_id, is_alive, is_processing, last_heartbeat, ...}], total, filter_applied, note}.\n\nFILTER: pass `filter` as a JSON OBJECT (not a string), e.g. {\"filter\": {\"status\": \"idle\"}} or {\"filter\": {\"in_dag\": \"dag-1\"}}. Valid status values: created|initializing|idle|starting|running|processing|stopping|terminated. Omit `filter` entirely to list all agents.\n\nTIP: To find agents you can talk to RIGHT NOW, look for agents with `can_receive_messages: true` or `availability: \"available\"`.",
